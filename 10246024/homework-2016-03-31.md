...
```
nil
profile裡面的是:name是一個KEY 不是"name"
```
...
```
num=1
File.open("homework-2016-03-31-content.txt") do |result|
    File.open("result.txt", "w") do |write|
...
p f.read.delete(",").delete(".").split(" ").select do |s| s.length <= 5 end.count()
f.close
```
