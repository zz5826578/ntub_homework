...
會得到什麼結果? 為什麼?
```
執行出來是nil，因為name是一個key而不是一個value，讀取name字串會讀不到東西，要用 p profile[:name] 才能正確讀取
```
...
i=0
output =""
File.open("homework-03-31-context.txt") do |f|
    f.each_line do |line|
             i=i+1
	     output += i.to_s + " #{line"}
    end			  
...
File.open("homework-03-31-content.txt") do |f|
    list=f.readlines("homework-03-31-content.txt").reverse.join
        list.split("\n").each do |line|
	        i+=1 if line.length <= 5
	end
end
puts i
```
